...
public class Controlador  implements ActionListener, MouseMotionListener, MouseListener{
	 ...
	 * @param diametro
	 */
	public Controlador(VistaJuego juego, InfoPanel info, int diametro, int nFilas) {
		setInfo(info);
		setJuego(juego);
		...
		tm = new Timer(7, new Next());
		setPinpon(Applet.newAudioClip(getClass().getResource("sound/pinpon.wav")));
		setFail(Applet.newAudioClip(getClass().getResource("sound/fail.wav")));
	}
	...
	public void setModelo(){
		setBolas(new BolasModelo(juego.getWidth(), diametro, nFilas));
	...
	class Next implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			int c = getBolas().colisiona(getTiro().getTiro(), getTiro().getColorTiro(), fin);
			if(c > 0){
				if(fin > 1){
					tm.stop();
					getTiro().reset();
