...
public class ListaPalabras {
	...
	public static void main(String[] args) {
		ListaPalabras histogram = new ListaPalabras(args[0]);
		histogram.write(args[1]);
	}

	public ListaPalabras (String inputFile){
	...
	public void write(String outputFile){
		...
		for (int i = 0; i < lines.size(); i++) {
			String[] linea = getLines(i).split(" ");
			for (int j = 0; j < linea.length; j++){
				if(caracter.containsKey(linea[j])){
					int value = caracter.get(linea[j]) + 1;
					caracter.put(linea[j], value);

				}else{
					caracter.put(linea[j], 1);		
					setMax(getMax() + 1);
				}
			}		
		}
